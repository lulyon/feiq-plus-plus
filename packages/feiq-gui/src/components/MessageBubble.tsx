      ...
      <div className={`max-w-[70%] ${isSent ? "order-1" : ""}`}>
        ...
        {message.contents.map((rawContent, i) => {
          ...
          if (content.type === "file") {
            // Detect folder transfer: filename starts with __FOLDER__
            const isFolder = content.filename?.startsWith("__FOLDER__");
            if (isFolder) {
              let folderMeta: { name?: string; count?: number; size?: number } = {};
              try {
                const json = content.filename!.slice("__FOLDER__".length);
                folderMeta = JSON.parse(json);
              } catch {}
              const folderName = folderMeta.name || "Folder";
              const fileCount = folderMeta.count || 0;
              const totalSize = folderMeta.size || 0;
              const canDownload = !isSent && !!content.localTaskId;

              const handleFolderClick = async () => {
                if (!canDownload) return;
                try {
                  const { open: folderDialogOpen } = await import("@tauri-apps/plugin-dialog");
                  const saveDir = await folderDialogOpen({
                    directory: true,
                    multiple: false,
              ...
              return (
                <div
                  ...
                  title={canDownload ? "Click to download folder" : undefined}
                >
                  <span className="group-hover:underline flex items-center gap-1.5">
                    <span>📁</span>
                    <span>{folderName}</span>
                    <span className="text-xs opacity-75">
                      ({fileCount} files{totalSize > 0 ? `, ${formatSize(totalSize)}` : ""})
                    </span>
                  </span>
                  {canDownload && (

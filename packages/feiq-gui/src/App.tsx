...
export default function App() {
  ...
  useEffect(() => {
    ...
    listen<{
      ...
      totalFiles: number;
    }>("folder-progress", (event) => {
      const {
        taskId, overallProgress, overallTotal,
        currentFile, currentFileProgress, currentFileTotal,
      ...
      upsertTransfer({
        ...
        direction: existing?.direction ?? "download",
        isFolder: true,
        folderName: existing?.folderName ?? existing?.filename ?? "",
        filesCompleted,
        totalFiles,
        currentFile,
        currentFileProgress,
    ...
    // Folder state changed events
    listen<{ taskId: number; state: string; message: string }>(
      "folder-state-changed",
      (event) => {
        const { taskId, state, message } = event.payload;
        const existing = useFileTransferStore.getState().transfers[taskId];
        const isFolder = existing?.isFolder ?? message.includes("folder");
        upsertTransfer({
          taskId,
          filename: existing?.filename ?? "",
          size: existing?.size ?? 0,
          progress: state === "finish" ? (existing?.size ?? 1) : (existing?.progress ?? 0),
          state: state as FileTransfer["state"],
          direction: existing?.direction ?? (message.startsWith("Sending") ? "upload" : "download"),
          isFolder,
          folderName: existing?.folderName ?? existing?.filename ?? "",
          filesCompleted: existing?.filesCompleted,
          totalFiles: existing?.totalFiles,
          currentFile: existing?.currentFile,
          currentFileProgress: existing?.currentFileProgress,
          currentFileTotal: existing?.currentFileTotal,
        });
      ...
      .onDragDropEvent((event) => {
        ...
        if (payload.type === "enter") {
          ...
          for (const filePath of payload.paths) {
            // Backend auto-detects file vs directory and routes accordingly
